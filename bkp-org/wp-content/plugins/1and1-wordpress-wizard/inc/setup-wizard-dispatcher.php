...
class One_And_One_Setup_Wizard_Dispatcher {
	...
	public function dispatch_wizard_actions() {
		...
		// Destroy sitetype options if wizard is restarted
		if(!isset($_REQUEST['setup_action']))
		{
			delete_option( 'oneandone_assistant_sitetype' );
			delete_option( 'oneandone_assistant_sitetype_translated' );
		}

		// Before the use can choose the design, a sitetype needs to be selected!
		if ( $this->is_action( 'choose_appearance' ) && isset( $_POST['sitetype'] ) ) {
			...
			if ( isset( $_POST['sitetypename'] ) AND ! empty( $_POST['sitetypename'][$site_type] ) ) {
				$site_type_translated = sanitize_text_field( $_POST['sitetypename'][$site_type] );
				update_option( 'oneandone_assistant_sitetype_translated', $site_type_translated );
			}
